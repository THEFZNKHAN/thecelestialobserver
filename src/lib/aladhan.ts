...
const getPrayerTimesByCoordinates = async (
    prefs: LocationPreferences,
): Promise<PrayerTimesResponse> => {
    const params = new URLSearchParams({
        latitude: String(prefs.latitude),
        longitude: String(prefs.longitude),
        method: String(prefs.method),
    });
    const payload = await fetchJson<unknown>(`${BASE_URL}/timings?${params.toString()}`);
    return prayerTimesResponseSchema.parse(payload);
};
...
export const getDashboardData = async (prefs: LocationPreferences): Promise<DashboardData> => {
    ...
    const prayerResponse = hasCoordinates
        ? await getPrayerTimesByCoordinates(prefs)
        : await getPrayerTimesByCity(prefs);
...
const getCalendarByCoordinates = async (
    prefs: LocationPreferences,
    month: number,
    year: number,
): Promise<CalendarByCityResponse> => {
    ...
    const payload = await fetchJson<unknown>(`${BASE_URL}/calendar?${params.toString()}`);
    return calendarByCityResponseSchema.parse(payload);
};
...
export const getMonthlyCalendar = async (
    ...
    const response = hasCoordinates
        ? await getCalendarByCoordinates(prefs, month, year)
        : await getCalendarByCity(prefs, month, year);
